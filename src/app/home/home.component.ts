...
export class HomeComponent implements OnInit {
 ...
 /*   const geoIpInfo = ""//this.storage.retrieve('geoIpInfo');
  ...
  save(model: any, isValid: boolean, e: any) {
    ...
    let companyName  = { name : model.name };
    let birthdate  = { countryName : model.countryName };
    let city  = { city : model.city };
    let zipCode  = { zipCode : model.zipCode };
    let street  = { street : model.street };
    let phone  = { street : model.street };
    let units = { units : model.units };

    this.database.createPatient(companyName, birthdate,
      city, zipCode, street, phone, units).subscribe(
      data => {
